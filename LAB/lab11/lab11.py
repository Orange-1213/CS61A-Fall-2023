...
def calc_apply(op, args):
    ...
    if op in OPERATORS:
        ...
        while args is not nil:
            result = OPERATORS[op](result, args.first)
            args = args.rest
        return result
    else:
        #print('error: unknown operator', op)
        if op in bindings:
            return bindings[op](args)
...
def floor_div(args):
    ...
    args = args.rest
    while args is not nil: 
        next_operand = calc_eval(args.first) 
        result //= next_operand  
        args = args.rest
    return result
...
OPERATORS = { "//": floor_div, "+": lambda x, y: x + y, "-": lambda x, y: x - y, "*": lambda x, y: x * y, "/": lambda x, y: x / y }
...
# print(calc_eval(Pair("d", Pair(4, Pair(2, nil)))))
