...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        location = self.place
        distance = 0
        while location and distance < self.lower_bound:
            location = location.entrance
            distance += 1
        while location and not location.is_hive and distance <= self.upper_bound:
            if location.bees: 
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while behind:
            if behind.ant:
                if not behind.ant.with_buff and not behind.ant.is_container:
                    behind.ant.double()
                elif behind.ant.is_container and behind.ant.ant_contained:
                    if not behind.ant.with_buff:
                        behind.ant.double()
                    if not behind.ant.ant_contained.with_buff:
                        behind.ant.ant_contained.double()
                elif behind.ant.is_container and not behind.ant.with_buff:
                    behind.ant.double()
            behind=behind.exit
