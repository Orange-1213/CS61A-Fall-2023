...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        value = scheme_eval(value_expression, env)
        ...
        current_formal = func_formals
        while current_formal is not nil:
            if not scheme_symbolp(current_formal.first):
                raise SchemeError('non-symbol: {0}'.format(current_formal.first))
            current_formal = current_formal.rest

        func_body = expressions.rest
        lambda_procedure = LambdaProcedure(func_formals, func_body, env)
...
def do_and_form(expressions, env):
    ...
    while current is not nil:
        last_val = scheme_eval(current.first, env)
        if is_scheme_false(last_val):
            return False
