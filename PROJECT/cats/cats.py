...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    after_select = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(after_select):
        return ''
    else:
        return after_select[k]
...
def about(subject):
    ...
    def select(paragraph):
        ...
        words = set(split(lower(cleaned_paragraph)))
        for word in subject:
            if word in words:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    differences = [(word, diff_function(typed_word, word, limit)) for word in word_list]
    
    differences.sort(key=lambda x: x[1])
    if differences[0][1] <= limit:
...
def fastest_words(match):
    ...
    for w_index in word_indices:
        word = get_word(match, w_index)
        
        fastest_time = float('inf')
        fastest_player = None
        for p_index in player_indices:
            t = time(match, p_index, w_index)
            if t < fastest_time:
                fastest_time = t
                fastest_player = p_index
        
        res[fastest_player].append(word)
